...
class PortraitBubbleView: View {
    ...
    public override fun onDraw(canvas: Canvas) {
        super.onDraw(canvas)
        if ((orientationValues[1].toDouble()+90) < 60) {
            drawFrame(canvas)
    ...
    private fun drawBubbleForFrontView(canvas: Canvas) {
        ...
        var pitch = (screenWidth / 2f - orientationValues[2]).toDouble()
        pitch = max((-10.0)+ screenWidth / 2f, pitch).toFloat().toDouble()
        pitch = min(screenWidth / 2f + 10.0, pitch).toFloat().toDouble()
    ...
    private fun drawBubbleForFlatSurface(canvas: Canvas) {
        ...
        canvas.drawCircle((screenWidth / 2).toFloat(), screenWidth / 2.0.toFloat(), 300f, paint)
        paint.color = Color.DKGRAY
        canvas.drawCircle((screenWidth / 2).toFloat(), screenWidth / 2.0.toFloat(), 75f, paint)
        val x = ((screenWidth / 2) - orientationValues[2]).toDouble()
        val y = ((screenWidth / 2) + orientationValues[1]).toDouble()
